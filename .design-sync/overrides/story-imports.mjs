...
// forked from design-sync lib/story-imports.mjs - domoticz-mobile is a React
// Native/Expo app: preview .tsx files import real components that import
// bare 'react-native' and, transitively (expo-modules-core), Metro-platform-
// suffixed siblings (`foo.web.ts` next to `foo.ts`) that only Metro resolves
// automatically. Added: rnWebPlugin, registered first in the plugin list.
// Everything else below is upstream, untouched.
import { existsSync, realpathSync } from 'node:fs';
import { relative, resolve } from 'node:path';

// React Native core -> react-native-web (exact bare specifier only), plus
// Metro's platform-suffix resolution for RELATIVE imports (the only shape
// this repo's dependency graph needs: expo-modules-core's own internal
// `./requireNativeModule` -> `./requireNativeModule.web.ts`).
function rnWebPlugin() {
  return {
    name: 'rn-web-alias',
    setup(b) {
      b.onResolve({ filter: /^react-native$/ }, async (args) => {
        if (args.pluginData === 'rn-web-resolving') return null;
        const r = await b.resolve('react-native-web', {
          kind: args.kind, resolveDir: args.resolveDir, pluginData: 'rn-web-resolving',
        });
        return r.errors.length ? null : { path: r.path };
      });
      b.onResolve({ filter: /^\.\.?\// }, (args) => {
        if (args.namespace && args.namespace !== 'file') return null;
        const cand = resolve(args.resolveDir, args.path);
        for (const suf of ['.web.ts', '.web.tsx', '.web.js', '.web.jsx']) {
          if (existsSync(cand + suf)) return { path: cand + suf };
        }
        return null;
      });
    },
...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  return {
    plugins: [rnWebPlugin(), dsShim, storybookStubPlugin(), consoleStub, policyRedirect],
