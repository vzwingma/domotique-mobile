// forked from design-sync lib/bundle.mjs - domoticz-mobile is a React Native
// (Expo) app, not a web package: its components import bare 'react-native'.
// esbuild has no notion of Metro's platform resolution, so without an alias
// to 'react-native-web' the bundle either fails to resolve or pulls in
// native-only code that can't run in a browser. Only change from upstream:
// `alias` added to sharedBuildOptions. Header/IIFE/footer contract untouched.
//
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    target: 'es2020',
    nodePaths: [nodePaths],
    // React Native core -> its web reimplementation. Exact-specifier only
    // (esbuild's `alias` doesn't rewrite deep subpath imports); the synced
    // component set was scoped to avoid libraries that need Metro's
    // platform-suffix (.web.js) resolution on top of this.
    alias: { 'react-native': 'react-native-web' },
    // Expo/RN packages ship Metro-platform-suffixed siblings (e.g.
    // requireNativeModule.web.ts next to requireNativeModule.ts) that Metro's
    // resolver picks automatically; esbuild needs this spelled out.
    ...
    loader: {
      ...
      '.woff': 'dataurl',
      '.woff2': 'dataurl',
      // @expo/vector-icons ships .ttf glyph fonts loaded via require(...).
      '.ttf': 'dataurl',
      // its vendored react-native-vector-icons source is plain .js containing
      // JSX (no separate .jsx extension) - esbuild's default 'js' loader for
      // .js doesn't parse JSX.
      '.js': 'jsx',
    },
    ...
    // Metro's InitializeCore normally injects these RN globals before any
    // module runs; a raw esbuild IIFE has no such bootstrap, so expo-modules-
    // core/react-native-web read straight through to a ReferenceError
    // (`process.env.EXPO_OS`, bare `global`, `__DEV__`) the moment they're
    // touched, which crashes the whole IIFE before window.<GLOBAL> is ever
    // assigned. Only defines what's missing - never overwrites a real global.
    banner: {
      js: 'var global = typeof global!=="undefined"?global:window;' +
        'var __DEV__ = typeof __DEV__!=="undefined"?__DEV__:true;' +
        'var process = typeof process!=="undefined"?process:{env:{NODE_ENV:"development",EXPO_OS:"web"}};',
    },
  };
}
