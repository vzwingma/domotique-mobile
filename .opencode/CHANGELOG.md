...
- **v2.2 → v2.3** : Index plans simplifié (sans phases) + màj obligatoire `.opencode/plans/README.md` lors changement statut plan.
...
- **v2.4 → v2.5** : Extraction procédures Plans Action et /fleet en skills partagés (`.opencode/skills/`). Sections AP et /fleet réduites aux spécificités ARCos (orchestration, création plan).
...
- **v4.5 → v4.6** : Suppression des blocs inline « opérations destructives » + « `.gitignore` » (désormais couverts par skills `safety-rules` + `copilotignore`, `applyTo: **`). Remplacés par un rappel d'une ligne.
...
- **v2.0 → v2.1** : Ajout règle synchro obligatoire `.opencode/plans/README.md` (index plans + statut global uniquement).
...
- **v4.2 → v4.3** : Suppression des blocs inline « opérations destructives » + « `.gitignore` » (couverts par skills `safety-rules` + `copilotignore`, `applyTo: **`).
...
- **v2.1 → v2.2** : Ajout règle maintenance `.opencode/plans/README.md` (index plans + statut global seulement).
