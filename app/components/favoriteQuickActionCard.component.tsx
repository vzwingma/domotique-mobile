...
import { DomoticzDeviceStatus, DomoticzDeviceType } from '../enums/DomoticzEnum';
 ...
 * Conçue pour l'écran Favoris : pas de slider, juste action principale + statut utile.
 */
export const FavoriteQuickActionCard: React.FC<FavoriteQuickActionCardProps> = ({ device }) => {
  ...
  const isActive = device.status !== DomoticzDeviceStatus.OFF && device.level > 0;
  const statusLabel = getStatusLabel(device, getLevel(device), false);
  const actionLabel = getFavoriteActionLabel(device);
...
function getFavoriteActionLabel(device: DomoticzDevice): string {
  const isOn = device.status !== DomoticzDeviceStatus.OFF && device.level > 0;

  if (device.type === DomoticzDeviceType.VOLET) {
    return isOn ? 'Fermer' : 'Ouvrir';
  }

  return isOn ? 'Éteindre' : 'Allumer';
}
