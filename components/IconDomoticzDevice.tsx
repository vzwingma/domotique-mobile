...
import { IconVoletSVG } from "@/components/IconVoletSVG";
...
export const IconDomoticzDevice : React.FC<IconDomoticzDeviceInternalProps> = ({ device, interactive = true } : IconDomoticzDeviceInternalProps) => {
  ...
  switch (device.type) {
    ...
    case DomoticzDeviceType.VOLET:
      if (!interactive) {
        return <IconVoletSVG device={device} />;
      }
      return (
        <Pressable onPress={() => performDevicePrimaryAction(device, setDomoticzDevicesData)}>
          <IconVoletSVG device={device} />
        </Pressable>
      );
...
/**
 * Retourne le nom de l'icône MaterialCommunityIcons pour une lumière.
 */
